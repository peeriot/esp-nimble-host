...
/// Load nimble-config.toml from the consuming crate, or fall back to defaults.
fn load_config() -> NimbleConfig {
    // Check the consuming crate's manifest dir first (set by cargo for -sys crates).
    // If not found, check esp-nimble-host's own directory.
    let search_paths = [
    ...
    for path in search_paths.iter().flatten() {
        let config_path = path.join("nimble-config.toml");
    ...
    println!("cargo:warning=No nimble-config.toml found, using defaults");
...
fn generate_syscfg_override(config: &NimbleConfig, out_dir: &Path) -> PathBuf {
    ...
    writeln!(h, "/* Generated by esp-nimble-host build.rs from nimble-config.toml */").unwrap();
    ...
    writeln!(h, "#define MYNEWT_VAL_BLE_ROLE_CENTRAL ({})", config.roles.central as u8).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_ROLE_OBSERVER ({})", config.roles.observer as u8).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_ROLE_PERIPHERAL ({})", config.roles.peripheral as u8).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_ROLE_BROADCASTER ({})", config.roles.broadcaster as u8).unwrap();
    writeln!(h).unwrap();

    // Connections
    writeln!(h, "/* Connections */").unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_MAX_CONNECTIONS ({})", config.connections.max_connections).unwrap();
    writeln!(h).unwrap();

    // Transport
    writeln!(h, "/* Transport buffers */").unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_TRANSPORT_ACL_COUNT ({})", config.transport.acl_count).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_TRANSPORT_ACL_FROM_HS_COUNT ({})", config.transport.acl_count).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_TRANSPORT_ACL_FROM_LL_COUNT ({})", config.transport.acl_count).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_TRANSPORT_ACL_SIZE ({})", config.transport.acl_size).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_TRANSPORT_EVT_COUNT ({})", config.transport.evt_count).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_TRANSPORT_EVT_DISCARDABLE_COUNT ({})", config.transport.evt_discardable_count).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_TRANSPORT_EVT_SIZE ({})", config.transport.evt_size).unwrap();
    writeln!(h).unwrap();

    // Msys
    writeln!(h, "/* System mbuf pool */").unwrap();
    writeln!(h, "#define MYNEWT_VAL_MSYS_1_BLOCK_COUNT ({})", config.msys.block_count).unwrap();
    writeln!(h, "#define MYNEWT_VAL_MSYS_1_BLOCK_SIZE ({})", config.msys.block_size).unwrap();
    writeln!(h).unwrap();

    // GATT
    writeln!(h, "/* ATT / GATT */").unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_ATT_PREFERRED_MTU ({})", config.gatt.preferred_mtu).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_GATT_MAX_PROCS ({})", config.gatt.max_procs).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_ATT_SVR_MAX_PREP_ENTRIES ({})", config.gatt.max_prep_entries).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_GATT_RESUME_RATE ({})", config.gatt.resume_rate_ms).unwrap();
    writeln!(h).unwrap();

    // L2CAP
    writeln!(h, "/* L2CAP */").unwrap();
    if config.l2cap.max_channels > 0 {
        writeln!(h, "#define MYNEWT_VAL_BLE_L2CAP_MAX_CHANS ({})", config.l2cap.max_channels).unwrap();
    } else {
        writeln!(h, "#define MYNEWT_VAL_BLE_L2CAP_MAX_CHANS (3*MYNEWT_VAL_BLE_MAX_CONNECTIONS)").unwrap();
    }
    writeln!(h, "#define MYNEWT_VAL_BLE_L2CAP_SIG_MAX_PROCS ({})", config.l2cap.max_sig_procs).unwrap();
    writeln!(h).unwrap();

    // Storage
    writeln!(h, "/* Bonding / Storage */").unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_STORE_MAX_BONDS ({})", config.storage.max_bonds).unwrap();
    writeln!(h, "#define MYNEWT_VAL_BLE_STORE_MAX_CCCDS ({})", config.storage.max_cccds).unwrap();
    writeln!(h).unwrap();
